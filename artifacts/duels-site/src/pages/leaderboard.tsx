...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    fetch("https://ipapi.co/json/")
      .then((r) => r.json())
      .then((d: any) => setRegion(EU_COUNTRIES.has(d.country_code) ? "EU" : "US"))
      .catch(() => setRegion("US"));
  }, []);
  ...
  const activeMode = MODES.find((m: { key: string; label: string }) => m.key === activeTab);
      ...
      <section className="relative z-10 pt-12 pb-10 sm:pt-16 sm:pb-12 text-center overflow-hidden">
        ...
        <div className="relative">
          ...
          {region && (
            <div className="mt-5">
              <span
                className={`inline-flex items-center gap-2 text-xs font-bold font-mono px-4 py-2 rounded-full border tracking-widest uppercase ${
                  region === "EU"
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-3 sm:px-5 pb-14">
        <div ref={tabsRef} className="glass-tabs rounded-2xl p-2 mb-5 overflow-x-auto tabs-scroll">
          <div className="flex gap-1.5 min-w-max sm:min-w-0 sm:flex-wrap">
            ...
            {MODES.map((m: { key: string; label: string }) => (
              <button
                key={m.key}
                onClick={() => setActiveTab(m.key as ModeKey)}
                ...
                <h2 className="text-lg sm:text-xl font-black text-foreground tracking-tight flex items-center gap-2.5">
                  {activeTab === "overview" ? (
                    ...
                    <>
                      ...
                      {activeMode?.label ?? activeTab} Rankings
                    </>
                  )}
                </h2>
